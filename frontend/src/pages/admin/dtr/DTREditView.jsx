...
function mergeDtrWithFullCalendarRange(formattedRows, maintenanceMap = {}) {
  ...
  const allKeys = [
    ...new Set([...byKey.keys(), ...Object.keys(maintenanceMap)]),
  ].sort();
  ...
  for (const key of allKeys) {
    ...
    if (byKey.has(key)) {
      ...
      out.push({
        rawDate: key,
        date: `${(dateObj.getMonth() + 1).toString().padStart(2, "0")}/${dateObj
          .getDate()
          .toString()
    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden max-w-6xl mx-auto border border-gray-100 flex flex-col h-[calc(80vh-80px)] min-h-[300px]">
      ...
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 max-w-sm w-full">
          ...
          >
            <div className="font-semibold mb-1">{toast.message}</div>

            {toast.details && (
              <pre className="text-xs whitespace-pre-wrap mt-2 max-h-40 overflow-y-auto">
                {toast.details}
              </pre>
            )}
          </div>
