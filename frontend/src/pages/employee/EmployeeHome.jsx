...
export default function EmployeeHome() {
  ...
  // Group files by year
  const grouped = dtrData.reduce((acc, file) => {
    const year = file.year || "Others";
     ...
     <div className="relative bg-surface w-full text-theme p-2 pt-2 overflow-y-hidden">
      <div className="p-1 md:p-5 md:mt-0">
        <div className="flex flex-row md:items-center justify-between mb-6 gap-4">
          ...
          <span className="text-2xl text-[#222] font-medium">{today}</span>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {Object.keys(grouped).length === 0 && (
              <div className="text-gray-500 text-sm">No DTR files found.</div>
            )}
