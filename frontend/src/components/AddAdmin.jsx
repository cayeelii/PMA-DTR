...
const AddAdminModal = ({
  ...
  const hasSingleRoleOption = roleOptions.length === 1;
  const autoAssignedRole = roleOptions[0] || "Admin";
  ...
  useEffect(() => {
    if (hasSingleRoleOption) {
      setFormData((prev) => ({ ...prev, role: autoAssignedRole }));
    }
  }, [autoAssignedRole, hasSingleRoleOption]);
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div
            className={`grid grid-cols-1 gap-5 ${
              hasSingleRoleOption ? "" : "md:grid-cols-2"
            }`}
          >
            <div>
